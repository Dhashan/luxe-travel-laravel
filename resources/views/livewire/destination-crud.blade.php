<div class="px-6 lg:px-12 font-sans selection:bg-yellow-200">
    ...
    <header class="mb-12 lg:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
            <p class="text-yellow-600 text-[10px] font-black uppercase tracking-[0.4em] mb-4">Inventory Control</p>
            <h2 class="text-4xl lg:text-6xl font-black text-gray-900 tracking-tighter">Global <span class="text-yellow-600 italic">Havens.</span></h2>
        </div>
    ...
    <!-- Add/Edit Modal (Simulated as inline for now) -->
    <div class="mb-20 @if(!$isOpen) hidden @endif animate-in fade-in slide-in-from-top-8 duration-700">
        <div class="bg-white rounded-[2rem] lg:rounded-[3.5rem] p-8 lg:p-16 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.05)] border border-gray-50">
            <h3 class="text-3xl lg:text-4xl font-black tracking-tighter mb-12 text-gray-900">{{ $destinationId ? 'Refine' : 'Register' }} <span class="text-yellow-600 italic">Haven.</span></h3>
            
            <form wire:submit.prevent="store" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Title</label>
                        <input type="text" wire:model="name" required 
                               ...
                               placeholder="e.g. Monte Carlo Riviera">
                    ...
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Base Valuation (USD)</label>
                        <input type="number" wire:model="base_price" step="1" required 
                               class="w-full bg-gray-50/50 border-2 border-transparent px-8 py-5 rounded-full text-sm font-bold focus:bg-white focus:border-yellow-600 focus:ring-0 transition duration-300"
                               placeholder="e.g. 15000">
                        @error('base_price') <span class="text-red-500 text-[10px] font-bold uppercase tracking-widest mt-2 ml-4 block italic">{{ $message }}</span> @enderror
                    </div>
                </div>
                
                <div class="group">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Thematic Narrative</label>
                    <textarea wire:model="description" rows="4" required 
                              ...
                              placeholder="Describe the atmosphere and exclusivity of this location..."></textarea>
                ...
                <div class="group">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Visual Asset Link</label>
                ...
                <div class="flex flex-col sm:flex-row gap-6 pt-4">
                    <button type="submit" class="px-10 py-5 bg-yellow-600 text-white rounded-full text-xs font-black uppercase tracking-widest hover:bg-yellow-700 transition duration-300 shadow-xl shadow-yellow-600/10">
                        {{ $destinationId ? 'Execute Refinement' : 'Confirm Registration' }}
                    </button>
            ...
            <table class="w-full text-left min-w-[800px] lg:min-w-0">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        <th class="px-8 lg:px-12 py-8 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Atmosphere</th>
                    ...
                    <tr class="group hover:bg-gray-50/50 transition">
                        <td class="px-8 lg:px-12 py-10">
                            <div class="flex items-center space-x-6">
                                <div class="w-24 h-16 rounded-2xl bg-gray-100 overflow-hidden shadow-inner flex-shrink-0">
                                    <img src="{{ $destination->image_url ?: 'https://images.unsplash.com/photo-1544124499-58912cbddaad?q=80&w=200' }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                                </div>
                                <div>
                                    <h4 class="text-lg font-black tracking-tighter text-gray-900 group-hover:text-yellow-600 transition">{{ $destination->name }}</h4>
                                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 line-clamp-1 max-w-xs">{{ Str::limit($destination->description, 60) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 lg:px-12 py-10">
                            <p class="text-xl font-black tracking-tighter text-gray-900">${{ number_format($destination->base_price) }}</p>
                            <p class="text-[8px] font-black uppercase tracking-widest text-yellow-600 italic">Premium Grade</p>
                        </td>
                        <td class="px-8 lg:px-12 py-10">
                            <p class="text-xs font-black text-gray-900 tracking-widest uppercase">{{ $destination->created_at ? $destination->created_at->format('M d, Y') : 'Eternal' }}</p>
                        </td>
                        <td class="px-8 lg:px-12 py-10 text-right">
                            <div class="flex justify-end gap-6 text-[10px] font-black uppercase tracking-widest transition italic">
                                <button wire:click="edit({{ $destination->id }})" class="text-yellow-600 hover:text-black">Edit &rarr;</button>
