<div class="px-6 lg:px-12 font-sans selection:bg-yellow-200">
    ...
    <header class="mb-12 lg:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
            <p class="text-yellow-600 text-[10px] font-black uppercase tracking-[0.4em] mb-4">Registry Control</p>
            <h2 class="text-4xl lg:text-6xl font-black text-gray-900 tracking-tighter">Client <span class="text-yellow-600 italic">Directory.</span></h2>
        </div>
    ...
    @if (session()->has('error'))
        <div class="mb-12 font-black text-[10px] uppercase tracking-widest text-red-600 text-center bg-red-50 py-5 rounded-full border border-red-100 italic animate-in slide-in-from-top-4 duration-500">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="mb-20 @if(!$isOpen) hidden @endif animate-in fade-in slide-in-from-top-8 duration-700">
        <div class="bg-white rounded-[2rem] lg:rounded-[3.5rem] p-8 lg:p-16 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.05)] border border-gray-50">
            <h3 class="text-3xl lg:text-4xl font-black tracking-tighter mb-12 text-gray-900">{{ $userId ? 'Refine' : 'Register' }} <span class="text-yellow-600 italic">Identity.</span></h3>
            
            <form wire:submit.prevent="store" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Full Legal Name</label>
                    ...
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Digital Identity (Email)</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Security Passkey {{ $userId ? '(Optional)' : '' }}</label>
                    ...
                    <div class="group">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-4">Access Clearance (Role)</label>
                        <select wire:model="role" required 
                            ...
                            <option value="user">Standard Voyager</option>
                            <option value="admin">Logistics Management</option>
                        </select>
                        @error('role') <span class="text-red-500 text-[10px] font-bold uppercase tracking-widest mt-2 ml-4 block italic">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-6 pt-4">
                    <button type="submit" class="px-10 py-5 bg-yellow-600 text-white rounded-full text-xs font-black uppercase tracking-widest hover:bg-yellow-700 transition duration-300 shadow-xl shadow-yellow-600/10">
                        {{ $userId ? 'Update Identity' : 'Confirm Registration' }}
                    </button>
            ...
            <table class="w-full text-left min-w-[800px] lg:min-w-0">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        <th class="px-8 lg:px-12 py-8 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Voyager Profile</th>
                        <th class="px-8 lg:px-12 py-8 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Clearance</th>
                        <th class="px-8 lg:px-12 py-8 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Timeline</th>
                    ...
                    <tr class="group hover:bg-gray-50/50 transition">
                        <td class="px-8 lg:px-12 py-10">
                            <div class="flex items-center space-x-6">
                                <div class="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center border border-gray-100 text-xs font-black text-gray-400 group-hover:bg-yellow-600 group-hover:text-white transition flex-shrink-0">
                                    {{ strtoupper(substr($user->name, 0, 2)) }}
                                </div>
                                <div>
                                    <h4 class="text-lg font-black tracking-tighter text-gray-900 group-hover:text-yellow-600 transition">{{ $user->name }}</h4>
                                    <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400">{{ $user->email }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 lg:px-12 py-10">
                        ...
                        <td class="px-8 lg:px-12 py-10">
                            <p class="text-xs font-black text-gray-900 tracking-widest uppercase">{{ $user->created_at ? $user->created_at->format('M d, Y') : 'Eternal' }}</p>
                            <p class="text-[8px] font-bold uppercase tracking-[0.2em] text-gray-300">Registry Date</p>
                        </td>
                        <td class="px-8 lg:px-12 py-10 text-right">
                            <div class="flex justify-end gap-6 text-[10px] font-black uppercase tracking-widest transition italic">
                                <button wire:click="edit({{ $user->id }})" class="text-yellow-600 hover:text-black">Edit &rarr;</button>
